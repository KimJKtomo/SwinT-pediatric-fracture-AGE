...
class Args:
    ...
    run_name = "Hospital_Train_ImageLevel"
    model_path = "swinT_hospital_best.pt"
...
def evaluate(model, loader, args, save_roc=False, name="val"):
    ...
    pred_bin = [1 if p > args.threshold else 0 for p in probs]
    ...
    if save_roc:
        ...
        plt.legend(); plt.grid()
        fname = f"roc_curve_{name}.png"
        plt.savefig(fname); mlflow.log_artifact(fname)
    return acc, f1, cm
...
def train():
    ...
    df = df[df['source'] == 'hospital'].reset_index(drop=True)
    print(f"\n📊 Dataset Summary:")
    ...
    train_loader = DataLoader(UnifiedFractureDataset(train_df, tf), batch_size=args.batch_size, sampler=sampler)
    val_loader = DataLoader(UnifiedFractureDataset(val_df, tf), batch_size=args.batch_size, shuffle=False)
    test_loader = DataLoader(UnifiedFractureDataset(test_df, tf), batch_size=args.batch_size, shuffle=False)
